...
- 入口は、タイムラインの上部固定ヘッダーの「プロフィール」（自分のプロフィールへ）と、タイムラインとポスト詳細の投稿者（アイコンと名前。その人のプロフィールへ）。ヘッダーには「ログアウト」も置く。
...
- タイムラインの変更は、ヘッダーの右側を「プロフィール」「ログアウト」の2項目にする点と、ポストの投稿者（アイコンと名前）をリンクにする点だけ。タイムラインのポスト1件の表示は、`posts/_post.blade.php` に切り出して、プロフィールのポスト一覧と共有する（出力は変えない）。
...
- ヘッダー（タイムライン）の右側に、「プロフィール」（自分のアイコン28pxつき。自分のプロフィールへのリンク）と「ログアウト」を置く。ヘッダーは `position: sticky; top: 0` で、スクロールしても上部に固定される。
- 以前ヘッダーに出していた「名前（メール）」の文字は、出さない（メールアドレスを画面に出し続けないため）。
- ログアウトは `POST`（Fortify の `logout` ルート）なので、リンクの見た目にしたボタンで送る。
...
- ヘッダーに、自分のプロフィールへのリンク（「プロフィール」）とログアウト（`POST` のフォーム）が出る。ログアウトすると未ログインに戻る。
- ヘッダーは上部に固定される（`position: sticky; top: 0`）。ヘッダーにメールアドレスは出ない。
- ヘッダーに自分のアイコン（28px）が出る。アバターを設定していれば画像、していなければ頭文字（`AvatarTest`）。
