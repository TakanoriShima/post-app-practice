...
- **テスト**: `tests/Feature/` に機能ごとのテストがある（`PostContentLimitTest`、`PostCreateTest`、`ReplyTest`、`ProfileTest`、`AvatarTest`、`LoginRedirectTest`）。いずれも `RefreshDatabase` を使う。アバターのテストは `Storage::fake('public')` で、実際のファイルは作らない。

## `docs/` フォルダ

機能ごとの設計書（Issue に対応）。`reply.md`（リプライ、#1）、`profile.md`（プロフィール、#2）、`avatar.md`（アバター画像、#3）、`post-create.md`（新規投稿、#8）。見出しは、概要 / 受け入れ条件 / 変えないもの / 増えるテーブル（Mermaid の `erDiagram` つき）/ 増える画面 / 入力チェックとメッセージ一覧 / テスト観点。機能を変えるときは、設計書も合わせて更新する。
